...
# wound_audit_tool.py - AI-Powered CMS-Grade Wound Note Audit Tool
...
st.markdown("""
This enhanced tool performs a detailed CMS-style audit of your wound care notes. It checks compliance with all major LCDs (L35125, L35041), CPT/ICD alignment, simulates fraud-detection AI logic used by Medicare auditors, and includes image upload for wound assessment + smart field prompts.
""")

# Upload or paste wound care note
uploaded_file = st.file_uploader("Upload Wound Note (.txt, .docx preferred)", type=["txt", "docx"])
note_text = ""
...
# Prompt builder
def build_audit_prompt(note: str, image_context: str = "") -> List[dict]:
    return [
        {"role": "system", "content": (
...
if st.button("Run Advanced Audit") and note_text:
    with st.spinner("Running CMS-grade AI audit with optional image support..."):
